...
vector<unsigned long> clb_req_zynq (MAX_SLOTS);
vector<unsigned long> bram_req_zynq (MAX_SLOTS);
vector<unsigned long> dsp_req_zynq (MAX_SLOTS);

Taskset *task_set;
Platform *platform;
vector <double> slacks = vector<double>(MAX_SLOTS);

vector <vector <unsigned long>> conn_matrix_zynq = vector <vector<unsigned long>> (MAX_SLOTS, vector<unsigned long> (MAX_SLOTS, 0));;
unsigned long num_conn_slots_zynq;

Vecpos fs_zynq(MAX_SLOTS);
...
*/
        ...
        // ------------------------------------------------------------
        // Constraint #1
        // Meaning: Every HW-Task must be allocated somewhere
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Constraint #2
        // Meaning: Decide wether a HW-task is subject to DPR
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Constraint #3
        // Meaning: Feasibility condition for the resources avaialable
        // on the FPGA
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Constraint #4
        // Meaning: Each slot must have enough resources to host all the
        ...
        // ------------------------------------------------------------
        // Constraint #5
        // Meaning: Bound the FPGA reconfiguration time
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Constraint #6
        // Meaning: Interference among HW-tasks due to execution
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Constraint #7
        // Meaning: Bound on delay incurred when requesting a HW-Task
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Constraint #8
        // Meaning: Enforce (delays <= given bound) to ensure schedulability
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Constraint #9
        // Meaning: Bound delay due to non-preemptive reconfiguration
        // ------------------------------------------------------------
/*
        if(!preemptive_FRI)
        {
...
//                model.addConstr(y[i] <= (H - beta[i][k] * (H - k)), "99");
                  model.addConstr(y[i] * num_rows + h[i] <= H, "100");
            //}
        }
*/
        for(i = 0; i < num_slots; i++) {
...
//#endif
            ...
            for(int a = 0; a < task_set->maxHW_Tasks; a++)
             ...
             //model.addConstr(50 * exp_res >= clb_req_zynq[i],"68");
             model.addConstr(50 * exp_res >= b[0][i],"68");
             model.addConstr(50 * exp_res <= BIG_M * exp_hw_task, "con hw1");
            //model.addConstr(wasted[i][0] == (50 * exp_res) - clb_req_zynq[i],"168"); //wasted clbs
            model.addConstr(wasted[i][0] == (50 * exp_res) - b[0][i],"168"); //wasted clbs
//#ifdef bram
            //model.addConstr(10 * exp_bram >= bram_req_zynq[i],"69");
            model.addConstr(10 * exp_bram >= b[1][i],"69");
            model.addConstr(50 * exp_bram <= BIG_M * exp_hw_task, "con hw1");
            //model.addConstr(wasted[i][1] == 10 * exp_bram - bram_req_zynq[i] ,"169");
            model.addConstr(wasted[i][1] == 10 * exp_bram - b[1][i] ,"169");

//#endif

//#ifdef dspp
            //model.addConstr(20 * exp_dsp >= dsp_req_zynq[i],"70");
            model.addConstr(20 * exp_dsp >= b[2][i],"70");
            model.addConstr(50 * exp_dsp <= BIG_M * exp_hw_task, "con hw1");
            //model.addConstr(wasted[i][2] == (20 * exp_dsp) - dsp_req_zynq[i], "170");
            model.addConstr(wasted[i][2] == (20 * exp_dsp) - b[2][i], "170");
//#endif
        }
//#endif
...
----------------------------------------------------------------------------------------------------\
------------------------  "<< endl;
            cout<< "slot \t" << "x_0 \t" << "y \t" << "w \t" << "h \t" << "clb \t" << "req_clb\t" << "bram \t"
                    << "req_bram\t" << "dsp\t" << "req_dsp" <<endl;
            cout << "----------------------------------------------------------------------\
--------------------------------------------------------------------------------------------\
-------------------------------" << endl;
            ...
            for(i = 0; i < num_slots; i++) {
                ...
                if(!active_partitions[i]){
                    (*to_sim->x)[i] = 0; //(int) x[i][0].get(GRB_DoubleAttr_X);
                    (*to_sim->y)[i] = 0; //(int) y[i].get(GRB_DoubleAttr_X) * 10;
                    (*to_sim->w)[i] = 0; //(int) w[i].get(GRB_DoubleAttr_X);
                    (*to_sim->h)[i] = 0; //(int) h[i].get(GRB_DoubleAttr_X) * 10
                }
                ...
                cout << i << "\t" << (*to_sim->x)[i] <<"\t"
                    ...
                    (clb[i][1].get(GRB_DoubleAttr_X) - clb[i][0].get(GRB_DoubleAttr_X)) *
                     h[i].get(GRB_DoubleAttr_X) * 50   << "\t" <<
                    task_set->HW_Tasks[i].resDemand[CLB] <<"\t" << (bram[i][1].get(GRB_DoubleAttr_X) -
                    bram[i][0].get(GRB_DoubleAttr_X)) * h[i].get(GRB_DoubleAttr_X) * 10 << "\t" << task_set->HW_Tasks[i].resDemand[BRAM]

                    << "\t" << (dsp[i][1].get(GRB_DoubleAttr_X) - dsp[i][0].get(GRB_DoubleAttr_X)) *
                        h[i].get(GRB_DoubleAttr_X) * 20 << "\t" << task_set->HW_Tasks[i].resDemand[DSP] <<endl;
...
int zynq_start_optimizer(param_to_solver *param, param_from_solver *to_sim)
{
    ...
    cout <<"resources inside optimizer" << endl;

    for(i = 0; i < num_slots; i++) {
        cout << "clb " << task_set->HW_Tasks[i].resDemand[CLB] <<
                "bram " << task_set->HW_Tasks[i].resDemand[BRAM] <<
                "dsp " << task_set->HW_Tasks[i].resDemand[DSP] <<
                "WCET " << task_set->HW_Tasks[i].WCET << endl;
    }
